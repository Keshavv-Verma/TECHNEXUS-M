...
void main() {
    ...
    // The cell is a perfect square cell in the atlas, and the image is already
    // contained, padded, and centered inside it. So we do direct mapping.
    ...
    // Smooth anti-aliasing on the edge of the circular disc
    float dist = length(vUvs - vec2(0.5));
    float antiAlias = 1.0 - smoothstep(0.48, 0.5, dist);
    outColor.a *= antiAlias;
    
    // Subtle 3D shading/inner shadow near the edges of the disc
    float innerShadow = 0.82 + 0.18 * smoothstep(0.5, 0.35, dist);
    outColor.rgb *= innerShadow;
...
class ArcballControl {
  constructor(canvas, updateCallback = null) {
    this.canvas = canvas;
  ...
  initTexture() {
    ...
    Promise.all(this.items.map((item) => loadSafeImage(item))).then((images) => {
      images.forEach((img, i) => {
        ...
        const targetSize = cellSize * scaleFactor;
        const imageAspect = img.width / img.height;
        let dw = targetSize;
        let dh = targetSize;

        if (imageAspect > 1) {
          dh = targetSize / imageAspect;
        } else {
          dw = targetSize * imageAspect;
        }
...
const InfiniteMenu = ({ items, scale = 1.0 }) => {
  ...
  return (
    <div className="infinite-menu-split-container">
      {/* 3D Sphere Interactive Area */}
      <div className="infinite-menu-canvas-wrapper">
        <canvas
          id="infinite-grid-menu-canvas"
          ref={canvasRef}
          className="infinite-menu-canvas"
        />
        <div className="infinite-menu-drag-hint">
          <span>← Drag to Rotate →</span>
        </div>
      </div>

      {/* Active Product Details Card */}
      <div className="infinite-menu-details-panel">
        {activeItem ? (
          <div className={`infinite-menu-details-card ${isMoving ? 'is-animating' : ''}`}>
            <div className="details-badge">Curated Item</div>
            <h3 className="details-title">{activeItem.title}</h3>
            
            <div className="details-price-row">
              <span className="price-label">ESTIMATED PRICE</span>
              <span className="price-value">{activeItem.description}</span>
            </div>

            <p className="details-spec-note">
              High-performance craftsmanship, selected for modern Obsidian workspaces and workspace refinement.
            </p>
          ...
          <div className="infinite-menu-loading">
            <p>Selecting curation...</p>
          </div>
        )}
      </div>
    </div>
